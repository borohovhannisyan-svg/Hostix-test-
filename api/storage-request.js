// Configure STORAGE_REQUEST_WEBHOOK_URL for the team's mail/ticket delivery service.
...
module.exports = async function handler(req, res) {
  ...
  const scenario = clean('scenario');
  const comment = clean('comment');
  const page = clean('page');
  const allowedProducts = new Set(['s3', 'ftp']);
  const allowedVolumes = new Set(['До 100 ГБ', '100–500 ГБ', '500 ГБ–2 ТБ', 'Более 2 ТБ', 'Пока не знаю']);
  const allowedScenarios = new Set(['Резервные копии', 'Файлы и архивы', 'Медиа и данные приложения', 'Обмен файлами', 'Другое']);
  if (!allowedProducts.has(product) || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254 || !allowedVolumes.has(volume) || !allowedScenarios.has(scenario) || comment.length > 2000 || page.length > 200) return res.status(400).json({accepted: false});

  const endpoint = process.env.STORAGE_REQUEST_WEBHOOK_URL;
  if (!endpoint) return res.status(503).json({accepted: false});
  try {
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {'Content-Type': 'application/json', ...(process.env.STORAGE_REQUEST_WEBHOOK_TOKEN ? {Authorization: `Bearer ${process.env.STORAGE_REQUEST_WEBHOOK_TOKEN}`} : {})},
      body: JSON.stringify({type: 'cloud-storage-request', product, email, volume, scenario, comment, page}),
