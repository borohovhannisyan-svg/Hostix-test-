// Configure SERVER_REQUEST_WEBHOOK_URL for the team's mail/ticket delivery service.
module.exports = async function handler(req, res) {
  ...
  if (!new Set(['colocation', 'windows-desktop', '1c-server']).has(product) || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254 || !task || task.length > 2000 || page.length > 200) return res.status(400).json({accepted: false});

  const endpoint = process.env.SERVER_REQUEST_WEBHOOK_URL;
  if (!endpoint) return res.status(503).json({accepted: false});
  try {
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {'Content-Type': 'application/json', ...(process.env.SERVER_REQUEST_WEBHOOK_TOKEN ? {Authorization: `Bearer ${process.env.SERVER_REQUEST_WEBHOOK_TOKEN}`} : {})},
      body: JSON.stringify({type: 'server-request', product, email, task, page}),
